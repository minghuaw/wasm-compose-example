...
impl ServerApp {
    ...
    async fn process_request(mut req: Request<State>) -> tide::Result {
        ...
        let response = store.data_mut().table().delete(host_response)?;
...
impl HostRequest for ServerWasiView {
    async fn new(
        ...
        let host_handle = self.table().push(MyRequest { headers, body })?;
    ...
    fn drop(&mut self, this: Resource<types::Request>) -> anyhow::Result<()> {
        ...
        Ok(self.table().delete(host_handle).map(|_| ())?)
    }
}
...
impl HostResponse for ServerWasiView {
    async fn new(
        ...
        let host_handle = self.table().push(MyResponse { headers, body })?;
        let guest_handle = Resource::new_own(host_handle.rep());
